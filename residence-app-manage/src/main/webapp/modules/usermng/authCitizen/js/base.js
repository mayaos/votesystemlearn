...
$(function(){
	...
	var grid = $('#gridList').datagrid({
		striped: true,
		dataType: 'json',
		url: Common.getUrl('citizenAuthctrl/list'),
		pagination: true,
		pagePosition: 'bottom',
		pageNumber: 1,
		toolbar: [{}],
	...
	$.ajax({
		...
		cache	: false,
		type	: 'POST',
		url		: Common.getUrl('citizenAuthctrl/getToolbar'),
		dataType: 'json',
		beforeSend: function() {
		},
	...
	$.ajax({
		...
		cache	: false,
		type	: 'POST',
		url		: Common.getUrl('citizenAuthctrl/maxImportDate'),
		dataType: 'json',
		beforeSend: function() {
		},
		success	: function(data) {
	...
	rcAuthForm.url = Common.getUrl('/citizenAuthctrl/citizenAuth');
	...
	$('#btnRcAuth').click(function() {
		...
		$.ajax({
			...
			cache	: false,
			type	: 'POST',
			url		: Common.getUrl('/citizenAuthctrl/batchCitizenAuth'),
			dataType: 'json',
			data	: {
	...
	function rcAuth() {
		...
		if (checkedItems.length > 0) {
			Common.confirm('确定要对当前选择的记录做居住证认证吗？', function(r) {
				if (r) {
					grid.datagrid('load');
					grid.datagrid('clearSelections');
					...
					$.ajax({
						...
						cache	: false,
						type	: 'POST',
						url		: Common.getUrl('/citizenAuthctrl/citizenAuth'),
						dataType: 'json',
						data	: {
							'authID': authID
						},
	...
	/*
	 * 临时处理程序
	 * 把已通过身份证认证但是未申请居住证认证的用户一次性进行居住证审核
	 */
	function residence() {
		$.ajax({
			timeout	: 15000,
			...
			cache	: false,
			type	: 'POST',
			url		: Common.getUrl('citizenAuthctrl/citizenAuthProvisional'),
			dataType: 'json',
			beforeSend: function() {
			},
			success	: function(data) {			
